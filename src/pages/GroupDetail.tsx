  ...
  return (
    <div className="min-h-screen bg-background pt-24 pb-12">
      ...
      <div className="container mx-auto max-w-6xl px-4">
        <div className="py-6">
          {/* Breadcrumb */}
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-6">
            <span 
          ...
          <div className="flex flex-col gap-6 mb-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={() => setShowScanBill(true)} variant="secondary" className="h-11 sm:h-12 gap-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-2xl border-b-2 border-muted active:border-b-0 active:translate-y-[1px] transition-all">
                <ScanLine className="w-3.5 h-3.5 sm:w-4 h-4" />
                Scan Bill
              </Button>
              <Button onClick={() => { setScannedData(undefined); setShowAddExpense(true); }} className="h-11 sm:h-12 gap-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-2xl border-b-2 border-primary-foreground/20 active:border-b-0 active:translate-y-[1px] transition-all shadow-xl shadow-primary/20">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button variant="outline" className="h-10 sm:h-11 gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] rounded-xl border-border/50" onClick={() => setShowSettleModal(true)}>
                <CheckCircle className="w-3 h-3 sm:w-3.5 h-3.5 text-emerald-500" />
                Settle Up
              </Button>
              <Button variant="outline" className="h-10 sm:h-11 gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] rounded-xl border-border/50" onClick={handleExportCSV}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            <div className="group relative overflow-hidden bg-card/40 backdrop-blur-xl rounded-3xl border border-border/50 p-4 xs:p-6 transition-all hover:shadow-2xl hover:shadow-primary/5">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-2">Total Budget</p>
              <p className="text-3xl sm:text-4xl font-black tracking-tight text-foreground">{formatCurrency(totalSpend)}</p>
            </div>
            <div className="group relative overflow-hidden bg-card/40 backdrop-blur-xl rounded-3xl border border-border/50 p-4 xs:p-6 transition-all hover:shadow-2xl hover:shadow-primary/5">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-2">Your Impact</p>
              <div className="flex flex-col xs:flex-row xs:items-end justify-between gap-3">
                <p className="text-3xl sm:text-4xl font-black tracking-tight text-foreground">{formatCurrency(yourShare)}</p>
                {isSettled ? (
                  <span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-4 py-1.5 rounded-full border border-emerald-500/20">
                    <CheckCircle className="w-3 h-3" />
                    Settled
                  </span>
                ) : yourBalance && yourBalance.netBalance > 0 ? (
                  <span className="inline-flex items-center text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-4 py-1.5 rounded-full border border-emerald-500/20 whitespace-nowrap">
                    Recieve {formatCurrency(yourBalance.netBalance)}
                  </span>
                ) : yourBalance ? (
                  <span className="inline-flex items-center text-[10px] font-black uppercase tracking-widest text-rose-500 bg-rose-500/10 px-4 py-1.5 rounded-full border border-rose-500/20 whitespace-nowrap">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <div className="flex items-center">
              <TabsList className="bg-muted/50 p-1 rounded-full h-11 border border-border/50 flex w-full">
                <TabsTrigger value="expenses" className="flex-1 px-2 sm:px-5 py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-full data-[state=active]:bg-background data-[state=active]:shadow-lg transition-all">Expenses</TabsTrigger>
                <TabsTrigger value="balances" className="flex-1 px-2 sm:px-5 py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-full data-[state=active]:bg-background data-[state=active]:shadow-lg transition-all">Balances</TabsTrigger>
                <TabsTrigger value="statistics" className="flex-1 px-2 sm:px-5 py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-full data-[state=active]:bg-background data-[state=active]:shadow-lg transition-all">Charts</TabsTrigger>
                <TabsTrigger value="activity" className="flex-1 px-2 sm:px-5 py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-full data-[state=active]:bg-background data-[state=active]:shadow-lg transition-all">History</TabsTrigger>
              </TabsList>
            </div>
            ...
            <TabsContent value="settings" className="mt-0 pt-2 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div className="space-y-6">
                ...
                {/* Group Members */}
                <div className="bg-card/50 backdrop-blur-sm rounded-3xl border border-border/50 p-6 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-4">Community</p>
                      ...
                      {group.members.map((member) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            {member === 'You' && (
                              <span className="text-[9px] font-black uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-full">Owner</span>
                            )}
                          </div>
                          {member !== 'You' && (
                            <Button
                ...
                <div className="bg-destructive/5 rounded-3xl border border-destructive/20 p-6">
                  <p className="text-[10px] font-black uppercase tracking-widest text-destructive mb-4">Danger Zone</p>
                  <AlertDialog>
